...
const getKeyboardData = async () => {
  ...
  $("div.grid-item__content").map((i, product) => {
    keyboards.push({
      name: $(product).find("div.grid-product__title").text().trim(),
      stock:
        $(product).find("div.grid-product__tags").text().trim() !== "sold out",
      productLink: `${domain}${$(product)
        .find("a.grid-item__link")
        .attr("href")}`,
      imageLink: `https:${cheerio
        .load(
          $(product)
            .find(".grid-product__image-wrap noscript")
            .first()
            .text()
            .trim()
        )("img")
      ...
      originalPrice: $(product)
        .find("span.grid-product__price--current .money")
        .first()
        .text()
        .trim(),
